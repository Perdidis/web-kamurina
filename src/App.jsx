...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handlePopState = (event) => {
      if (fotoAmpliada) { setFotoAmpliada(null); window.history.pushState({ vista }, ''); return; }
      if (modalConfirm.isOpen) { setModalConfirm({ isOpen: false, text: '', action: null, buttons: null }); window.history.pushState({ vista }, ''); return; }
      if (modalRechazo.isOpen) { setModalRechazo({ isOpen: false, pedidoId: null, motivo: '' }); window.history.pushState({ vista }, ''); return; }
      if (modalPago.isOpen) { setModalPago({ isOpen: false, pedidoId: null }); window.history.pushState({ vista }, ''); return; }
      if (menuAbierto) { setMenuAbierto(false); window.history.pushState({ vista }, ''); return; }
      ...
      setVista(event.state?.vista || 'dashboard');
      setFormDirty(false);
    };
  ...
  useEffect(() => {
    ...
    const unsubTelas = onSnapshot(collection(db, "telas"), (snapshot) => {
      setTelas(snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })));
    }, (err) => console.error("Error leyendo telas:", err));

    const unsubAvios = onSnapshot(collection(db, "avios"), (snapshot) => {
      setAvios(snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })));
    }, (err) => console.error("Error leyendo avios:", err));

    return () => { unsubClientes(); unsubPedidos(); unsubTelas(); unsubAvios(); };
  }, [user]);
  ...
  const borrarCliente = async (id) => {
    try {
      ...
      if (clienteABorrar) {
        ...
        await Promise.all(pedidosDelCliente.map(p => deleteDoc(doc(db, "pedidos", String(p.id)))));
      }
  ...
  const actualizarStock = async (id, nuevoStock) => {
    try {
      ...
      if (tela) await setDoc(doc(db, "telas", String(id)), { ...tela, stock: nuevoStock }, { merge: true });
  ...
  const actualizarStockAvio = async (id, nuevoStock) => {
    try {
      ...
      if (avio) await setDoc(doc(db, "avios", String(id)), { ...avio, stock: nuevoStock }, { merge: true });
    } catch (err) { console.error(err); }
  };

  // Validación y guardado con crypto.randomUUID()
  ...
  const guardarTela = async (e) => {
    ...
    if (precio < 0) { mostrarToast("⚠️ El precio no puede ser negativo"); return; }
    try {
      const id = crypto.randomUUID();
      const nueva = { id, nombre: fd.get('nombre'), descripcion: fd.get('desc'), uso: fd.get('uso'), stock: fd.get('stock'), precio, foto: fd.get('foto') };
      await setDoc(doc(db, "telas", id), nueva);
      mostrarToast("Tela agregada al catálogo");
      cambiarVista('catalogo');
    } catch (err) { mostrarToast("Error al guardar tela"); }
  };
  ...
  const actualizarTelaEditada = async (e) => {
    ...
    if (precio < 0) { mostrarToast("⚠️ El precio no puede ser negativo"); return; }
    try {
      const actualizada = { ...telaSeleccionada, nombre: fd.get('nombre'), descripcion: fd.get('desc'), uso: fd.get('uso'), stock: fd.get('stock'), precio, foto: fd.get('foto') };
    ...
    } catch (err) { mostrarToast("Error al actualizar tela"); }
  };
  ...
  const guardarAvio = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, "avios", id), nuevo);
      mostrarToast("Avío guardado");
      cambiarVista('catalogo-avios');
    } catch (err) { mostrarToast("Error al guardar avío"); }
  };
  ...
  const actualizarAvioEditado = async (e) => {
    ...
    try {
      const fd = new FormData(e.target);
      const actualizado = { ...avioSeleccionado, nombre: fd.get('nombre'), descripcion: fd.get('desc'), uso: fd.get('uso'), stock: fd.get('stock'), foto: fd.get('foto') };
      await setDoc(doc(db, "avios", String(avioSeleccionado.id)), actualizado);
      setAvioSeleccionado(actualizado);
      mostrarToast("Avío actualizado");
      cambiarVista('detalle-avio');
    } catch (err) { mostrarToast("Error al actualizar avío"); }
  };
  ...
  const crearPedido = async (e) => {
    ...
    try {
      ...
      if (!esAdmin && !/^\d{6,15}$/.test(telefonoCliente)) {
        mostrarToast("⚠️ Ingresa un teléfono válido (solo números)");
        setIsSaving(false);
        return;
      }
      ...
      const nuevo = { 
          id, createdAt: timestamp, cliente: nombreCliente, telefono: telefonoCliente,
          prenda: fd.get('prenda'), estado: estadoInicial, entrega: '', 
          descripcionDetalle: esAdmin ? '' : (fd.get('descripcionDetalle') || ''),
          precio: 0, pagado: false, pagos: [], tela: fd.get('tela') || '',
          foto: fd.get('foto') || '', fotos: fd.get('foto') ? [fd.get('foto')] : [],
          ocultoDashboard: false, materialesCosto: 0, manoObraCosto: 0, gastos: 0, motivoRechazo: ''
      };

      await setDoc(doc(db, "pedidos", id), nuevo);
      mostrarToast("¡Solicitud enviada con éxito!");

      if (!esAdmin) {
        const clienteEncontrado = clientes.find(c => c.nombre && c.nombre.toLowerCase() === nombreCliente.toLowerCase());
        if (!clienteEncontrado) {
          const nuevoClienteId = crypto.randomUUID();
          const medidasVacias = {};
          MEDIDAS_LISTA.forEach(m => medidasVacias[m] = '');
          await setDoc(doc(db, "clientes", nuevoClienteId), { id: nuevoClienteId, nombre: nombreCliente, telefono: telefonoCliente, medidas: medidasVacias });
        }
      }
  ...
  const asignarPrecioAPedido = async (e) => {
    ...
    if (precioFinal < 0) { mostrarToast("⚠️ El precio no puede ser negativo"); return; }
    try {
      const pedido = pedidos.find(p => p.id === pedidoId);
      if (pedido) {
        const actualizado = { ...pedido, precio: precioFinal, materialesCosto: materiales, manoObraCosto: manoObra, gastos: materiales };
        await setDoc(doc(db, "pedidos", String(pedidoId)), actualizado, { merge: true });
        mostrarToast("Precio asignado correctamente");
      }
      cambiarVista('dashboard');
    } catch (err) { mostrarToast("Error al asignar precio"); }
  };

  const ocultarPedidoDashboard = async (id) => {
    try {
      const pedido = pedidos.find(p => p.id === id);
      if (pedido) await setDoc(doc(db, "pedidos", String(id)), { ...pedido, ocultoDashboard: true }, { merge: true });
  ...
  const borrarPedidoDefinitivo = async (id) => {
    ...
    } catch (err) { mostrarToast("Error al borrar pedido"); }
  };
  ...
  const registrarPagoParcial = async () => {
    ...
    try {
      ...
      const nuevoPago = { id: Date.now(), monto, metodo: metodoPagoInput, fecha: new Date().toLocaleDateString() };
      ...
      const actualizado = { ...pedido, pagos: listaActualizada, pagado: estaPagado };
      await setDoc(doc(db, "pedidos", String(pedido.id)), actualizado, { merge: true });
      if (pedidoSeleccionado?.id === pedido.id) setPedidoSeleccionado(actualizado);
      
      setModalPago({ isOpen: false, pedidoId: null });
      setMontoPagoInput('');
      mostrarToast("Pago registrado con éxito");
    } catch (err) { mostrarToast("Error al registrar pago"); }
  };
  ...
  const eliminarPagoParcial = async (pagoId) => {
    ...
    try {
      ...
      const actualizado = { ...pedidoSeleccionado, pagos: listaActualizada, pagado: estaPagado };
    ...
    } catch (err) { mostrarToast("Error al eliminar pago"); }
  };
    ...
    try {
      ...
      if (pedido) {
        await setDoc(doc(db, "pedidos", String(modalRechazo.pedidoId)), { 
          ...pedido, estado: 'Rechazado', motivoRechazo: modalRechazo.motivo, ocultoDashboard: true 
        }, { merge: true });
      }
      setModalRechazo({ isOpen: false, pedidoId: null, motivo: '' });
      mostrarToast("Pedido rechazado");
    } catch (err) { mostrarToast("Error al rechazar pedido"); }
  };

  // Función para exportar reporte de Ganancias a PDF
  ...
  const handleEmailAuth = async (e) => {
    ...
    setError('');
    try {
      if (isLoginView) await signInWithEmailAndPassword(auth, loginUser, loginPass);
      else await createUserWithEmailAndPassword(auth, loginUser, loginPass);
    } catch (err) {
  ...
  const pedidosVisibles = pedidos.filter(p => {
    ...
    const coincideBusqueda = !textoBusqueda || p.cliente.toLowerCase().includes(textoBusqueda) || p.prenda.toLowerCase().includes(textoBusqueda) || p.id.toLowerCase().includes(textoBusqueda);
    return coincideFiltro && coincideBusqueda;
  }).sort((a, b) => (Number(b.createdAt) || 0) - (Number(a.createdAt) || 0));

  const solicitudesPendientesAdmin = pedidos.filter(p => !p.ocultoDashboard && p.estado === 'Pendiente de Aprobación');

  // Cálculos para KPIs en el Dashboard de Admin
  ...
  const ingresosDelMes = pedidos.reduce((acc, p) => {
    ...
    // Si tiene pagos registrados, sumamos lo abonado este mes, o si no, basado en fecha de creación/entrega
  ...
  if (!user) {
    return (
        <div translate="no" className="notranslate min-h-screen bg-stone-950 text-white flex items-center justify-center p-4 md:p-8 font-sans">
            <div className={`p-6 md:p-8 rounded-3xl w-full max-w-sm backdrop-blur-xl border ${isLoginView ? 'bg-stone-900/40 border-stone-800' : 'bg-stone-900/60 border-stone-600 shadow-2xl'}`}>
                <h1 className="text-3xl font-bold mb-1 text-center tracking-tighter">{isLoginView ? 'Atelier' : 'Nueva Cuenta'}</h1>
                <p className="text-center text-stone-400 text-sm mb-8">{isLoginView ? 'Ingresa para continuar' : 'Regístrate para solicitar pedidos'}</p>
                <form onSubmit={handleEmailAuth} className="space-y-4">
                    <input type="email" placeholder="Correo electrónico" value={loginUser} onChange={(e) => setLoginUser(e.target.value)} className="w-full bg-stone-950 p-3 rounded-xl border border-stone-800 outline-none focus:border-stone-500" required />
                    <input type="password" placeholder="Contraseña" value={loginPass} onChange={(e) => setLoginPass(e.target.value)} className="w-full bg-stone-950 p-3 rounded-xl border border-stone-800 outline-none focus:border-stone-500" required />
                    {error && <p className="text-red-400 text-xs text-center">{error}</p>}
                    <button type="submit" className={`w-full py-3 rounded-xl font-bold transition-all ${isLoginView ? 'bg-white text-stone-950 hover:bg-stone-200' : 'bg-stone-800 text-white hover:bg-stone-700 border border-stone-600'}`}>
                        {isLoginView ? 'Iniciar Sesión' : 'Registrarme'}
                    </button>
                </form>
                <div className="mt-6 flex items-center justify-center space-x-2">
                    <div className="h-px bg-stone-800 w-full"></div><span className="text-xs text-stone-500 uppercase">O</span><div className="h-px bg-stone-800 w-full"></div>
                </div>
                <button onClick={handleGoogleLogin} type="button" className="w-full mt-6 bg-stone-950 text-white py-3 rounded-xl font-bold border border-stone-800 hover:bg-stone-900 flex items-center justify-center gap-2">
                ...
                <p className="mt-6 text-center text-sm text-stone-400">
                    ...
                    <button onClick={() => setIsLoginView(!isLoginView)} className="text-white hover:underline font-bold">{isLoginView ? 'Regístrate aquí' : 'Inicia sesión'}</button>
                </p>
            </div>
        </div>
    );
    ...
    <div translate="no" className="notranslate min-h-screen bg-stone-950 text-white p-4 md:p-8 font-sans selection:bg-white selection:text-stone-950">
      ...
      <style>{`
        @media print {
          ...
          .print-ficha-exclusiva, .print-ficha-exclusiva *, .print-ganancias-exclusiva, .print-ganancias-exclusiva * { visibility: visible !important; }
          .print-ficha-exclusiva, .print-ganancias-exclusiva {
            position: absolute !important; left: 0 !important; top: 0 !important; width: 100% !important; background: white !important; color: black !important; z-index: 999999; padding: 20px; display: block !important;
          }
        }
      `}</style>
      ...
      <main className="relative z-10 max-w-6xl mx-auto">
        {vista === 'dashboard' && (
          <div>
            {/* KPI CARDS ARRIBA DEL DASHBOARD PARA ADMIN */}
            {esAdmin && (
            ...
            <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="w-full md:w-auto flex-1">
                <input 
                  type="text" placeholder="Buscar pedido por prenda o ID..." value={busquedaDashboard}
              ...
              {!esAdmin && (
                <button onClick={() => cambiarVista('nuevo-pedido')} className="w-full md:w-auto bg-white text-stone-950 px-6 py-4 rounded-2xl font-bold text-sm whitespace-nowrap hover:bg-stone-200 transition-colors">
                  + Solicitar Pedido
                </button>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              {pedidosVisibles.length === 0 ? (
                ...
                pedidosVisibles.map(p => {
                  ...
                  return (
                    <div 
                      key={p.id} onClick={() => { setPedidoSeleccionado(p); cambiarVista('detalle-pedido'); }} 
                    ...
                    >
                      <button 
                        onClick={(e) => { 
                          ...
                          if (esAdmin) {
                            setModalConfirm({ 
                              isOpen: true, text: "¿Qué deseas hacer con este pedido?", 
                              buttons: [
                                { text: "Solo quitar del Dashboard", action: () => ocultarPedidoDashboard(p.id), style: "bg-stone-800 text-white hover:bg-stone-700" },
                                { text: "Eliminar definitivamente", action: () => borrarPedidoDefinitivo(p.id), style: "bg-red-950/40 text-red-400 border border-red-900/50 hover:bg-red-900/40" }
                              ]
                            }); 
                          } else {
                            setModalConfirm({ isOpen: true, text: "¿Estás seguro de que quieres eliminar esta solicitud de pedido?", action: () => borrarPedidoDefinitivo(p.id) });
                          }
                        }} 
                        ...
                        <div onClick={(e) => e.stopPropagation()} className="mb-2">
                          {editandoEstadoId === p.id ? (
                            <div className="flex gap-1 mt-1">
                              <input 
                                type="text" value={nuevoEstadoTexto} onChange={(e) => setNuevoEstadoTexto(e.target.value)}
                                className="w-full bg-stone-950 border border-stone-700 p-1.5 rounded-xl text-xs text-white outline-none" autoFocus
                              />
                              <button onClick={async () => { if (nuevoEstadoTexto.trim()) await actualizarEstado(p.id, nuevoEstadoTexto.trim()); setEditandoEstadoId(null); }} className="bg-white text-stone-950 px-2.5 rounded-xl text-xs font-bold">✓</button>
                              <button onClick={() => setEditandoEstadoId(null)} className="bg-stone-800 text-stone-400 px-2 rounded-xl text-xs">✕</button>
                            </div>
                          ) : (
                            <div className="flex items-center justify-between bg-stone-950/40 border border-stone-800/80 px-3 py-2 rounded-xl text-xs">
                              ...
                              <button onClick={() => { setEditandoEstadoId(p.id); setNuevoEstadoTexto(p.estado); }} className="text-stone-400 hover:text-white p-1" title="Editar estado">
                      ...
                      <div className="mb-4">
                        ...
                        {esAdmin && p.precio > 0 && <p className="text-xs text-emerald-400 font-medium">Ganancia: +${gananciaPedido.toLocaleString()}</p>}
                      </div>

                      {/* BOTÓN RÁPIDO "REGISTRAR PAGO" EN EL DASHBOARD (SOLO ADMIN) */}
                      {esAdmin && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="mb-2 w-full bg-stone-800 hover:bg-stone-700 text-white py-2 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-colors border border-stone-700"
                        >
                          💳 Registrar Pago / Seña
                        </button>
                      )}
                      ...
                      {(() => {
                        ...
                        const mensaje = esAdmin ? `Hola ${p.cliente}, te escribo desde Atelier por tu pedido de ${p.prenda}.` : `Hola, le escribo por mi pedido de ${p.prenda} (${p.id}).`;
                        const urlWhatsapp = `https://wa.me/${telefonoContacto.replace(/\D/g, '')}?text=${encodeURIComponent(mensaje)}`;

                        return (
                          <a href={urlWhatsapp} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="w-full bg-emerald-950/40 border border-emerald-900/50 text-emerald-300 py-2.5 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-emerald-900/40 transition-colors">
        ...
        {esAdmin && vista === 'solicitudes' && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              {solicitudesPendientesAdmin.length === 0 ? (
                <p className="col-span-full text-stone-500 text-center py-10 italic">No hay nuevas solicitudes de pedidos pendientes.</p>
              ) : (
                solicitudesPendientesAdmin.map(p => (
                  <div key={p.id} className="bg-stone-900/40 backdrop-blur-md border border-amber-900/50 p-6 rounded-3xl relative flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-4">
                        <span className="text-[10px] uppercase tracking-widest text-stone-500">{p.id}</span>
                        <span className="text-[10px] uppercase px-2 py-1 rounded bg-amber-950 text-amber-300 border border-amber-900/50">Pendiente de Aprobación</span>
                      </div>
                      ...
                      {p.descripcionDetalle && (
                        <div className="bg-stone-950/60 border border-stone-800 p-3 rounded-xl mb-3 text-xs text-stone-300">
                          <strong className="text-stone-400 block mb-1">Detalles:</strong> {p.descripcionDetalle}
                        </div>
                      )}
                    ...
                    <div className="flex gap-2 mt-4 pt-4 border-t border-stone-800">
                      <button onClick={() => setModalRechazo({ isOpen: true, pedidoId: p.id, motivo: '' })} className="flex-1 bg-red-950/40 text-red-400 border border-red-900/50 py-2.5 rounded-xl text-xs font-bold hover:bg-red-900/40">Rechazar</button>
                      <button onClick={() => aceptarSolicitud(p.id)} className="flex-1 bg-white text-stone-950 py-2.5 rounded-xl text-xs font-bold hover:bg-stone-200">Aceptar</button>
                    </div>
                  </div>
                ))
              )}
            ...
            <div className={`bg-stone-900/40 backdrop-blur-md border p-6 md:p-10 rounded-3xl max-w-3xl mx-auto relative ${esRechazado ? 'border-red-900/60' : 'border-stone-800'}`}>
              ...
              {esAdmin ? (
                <form onSubmit={async (e) => {
                    e.preventDefault();
                    const fd = new FormData(e.target);
                    const nuevoPrecio = Number(fd.get('precio'));
                    if (nuevoPrecio < 0) { mostrarToast("⚠️ El precio no puede ser negativo"); return; }
                    try {
                      const nuevoEstado = fd.get('estado');
                      const actualizado = {
                          ...pedidoSeleccionado, prenda: fd.get('prenda'), tela: fd.get('tela'),
                          precio: nuevoPrecio, gastos: Number(fd.get('gastos')) || 0,
                          estado: nuevoEstado, motivoRechazo: nuevoEstado === 'Rechazado' ? pedidoSeleccionado.motivoRechazo : '',
                      ...
                      cambiarVista('dashboard');
                    } catch (err) { mostrarToast("Error al actualizar pedido"); }
                }}>
                ...
                <div className="space-y-4 mb-6 text-sm bg-stone-950/50 p-4 rounded-2xl border border-stone-800">
                  ...
                  {pedidoSeleccionado.descripcionDetalle && <p><strong>Detalles:</strong> {pedidoSeleccionado.descripcionDetalle}</p>}
              ...
              {/* CONTROL DE PAGOS Y CUOTAS TÁCTIL */}
              <div className="bg-stone-950/80 border border-stone-800 p-6 rounded-2xl mb-6 shadow-inner">
                ...
                {pagosRealizados.length > 0 && (
                  <div className="space-y-2.5 mb-5">
                    <p className="text-xs text-stone-400 uppercase tracking-wider font-semibold">Historial de pagos:</p>
                    {pagosRealizados.map((pago) => (
                      <div key={pago.id} className="flex justify-between items-center bg-stone-900/90 p-3 rounded-xl border border-stone-800 text-xs">
                        ...
                        {esAdmin && <button onClick={() => eliminarPagoParcial(pago.id)} className="text-stone-400 hover:text-red-400 p-1 font-bold text-sm">✕</button>}
                      </div>
                    ))}
                  </div>
                )}
                {esAdmin && (
                  <button onClick={() => setModalPago({ isOpen: true, pedidoId: pedidoSeleccionado.id })} className="w-full bg-white text-stone-950 py-3.5 rounded-xl font-bold text-xs md:text-sm flex items-center justify-center gap-2 hover:bg-stone-200 transition-colors">
              ...
              {(() => {
                ...
                if (!telefonoContacto) return null;
                const mensaje = esAdmin ? `Hola ${pedidoSeleccionado.cliente}, te escribo desde Atelier por tu pedido de ${pedidoSeleccionado.prenda}.` : `Hola, le escribo por mi pedido de ${pedidoSeleccionado.prenda} (${pedidoSeleccionado.id}).`;
                const urlWhatsapp = `https://wa.me/${telefonoContacto.replace(/\D/g, '')}?text=${encodeURIComponent(mensaje)}`;
                return (
                  <a href={urlWhatsapp} target="_blank" rel="noopener noreferrer" className="mb-8 w-full bg-emerald-950/40 border border-emerald-900/50 text-emerald-300 py-3 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-emerald-900/40 transition-colors block text-center">
                    <svg className="w-4 h-4 fill-current inline-block" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
                    {esAdmin ? 'Abrir chat de WhatsApp' : 'Contactar por WhatsApp'}
                  </a>
                );
              })()}

              <h3 className="text-lg font-semibold mb-4">Fotos del Trabajo</h3>
              {arrayFotos.length === 0 ? <p className="text-stone-500 text-xs italic mb-4">No hay fotos guardadas.</p> : (
                <div className="flex gap-2 overflow-x-auto pb-4 mb-4">
                  {arrayFotos.map((img, i) => (
                    <img key={i} src={img} alt="Trabajo" className="w-32 h-32 object-cover rounded-xl border border-stone-800 flex-shrink-0 cursor-pointer hover:opacity-80" onClick={() => setFotoAmpliada(img)} />
                  ))}
                </div>
              )}
              ...
              {esAdmin && (
                <form onSubmit={async (e) => {
                    ...
                    try {
                      ...
                      const actualizado = { ...pedidoSeleccionado, fotos: [...arrayFotos, url] };
                      await setDoc(doc(db, "pedidos", String(pedidoSeleccionado.id)), actualizado, { merge: true });
                      setPedidoSeleccionado(actualizado);
                      e.target.reset();
                      mostrarToast("Foto agregada");
                    } catch (err) { mostrarToast("Error al agregar foto"); }
                    ...
                    <button type="submit" className="bg-white text-stone-950 px-4 py-3 rounded-xl text-sm font-bold">Agregar</button>
                </form>
              )}
            </div>
          );
        ...
        {vista === 'nuevo-pedido' && (
           <form onSubmit={crearPedido} className="bg-stone-900/40 p-6 md:p-8 rounded-3xl border border-stone-800 max-w-lg mx-auto">
             ...
             {!esAdmin && (() => {
               const clienteExistente = clientes.find(c => c.nombre && c.nombre.toLowerCase() === (user.displayName || user.email).toLowerCase());
               if (clienteExistente?.telefono) return <input type="hidden" name="telefono" value={clienteExistente.telefono} />;
               return <input name="telefono" placeholder="Teléfono Móvil (Ej: 3434...)" className="w-full bg-stone-950 p-3 rounded-xl mb-4 border border-stone-800 outline-none" required />;
             })()}
             <div>
               <label className="block text-xs text-stone-400 mb-1">Descripción del pedido</label>
               <textarea name="descripcionDetalle" rows="3" placeholder="Color, forma, tipo de tela..." className="w-full bg-stone-950 p-3 rounded-xl mb-4 border border-stone-800 outline-none text-sm text-white resize-none" required />
             </div>
             {esAdmin && (
               <>
             ...
             <div className="flex gap-3">
               <button type="button" onClick={() => cambiarVista('dashboard')} className="w-full bg-stone-800 text-white py-3 rounded-xl font-bold">Cancelar</button>
               <button type="submit" disabled={isSaving} className="w-full bg-white text-stone-950 py-3 rounded-xl font-bold">{isSaving ? 'Enviando...' : 'Enviar Solicitud'}</button>
             </div>
           </form>
        )}
        ...
        {esAdmin && vista === 'catalogo' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {telas.length === 0 ? <p className="col-span-full text-stone-500 text-center py-10 italic">No existen telas registradas.</p> : (
                  telas.map(t => (
                    <div key={t.id} className="bg-stone-900/40 backdrop-blur-md border border-stone-800 rounded-3xl overflow-hidden relative">
                      <button onClick={(e) => { e.stopPropagation(); setModalConfirm({ isOpen: true, text: "¿Estás segura de que quieres eliminar esta tela?", action: () => borrarTela(t.id) }); }} className="absolute top-2 right-2 text-white bg-black/50 p-2 rounded-full hover:bg-red-900 z-10 text-xs">✕</button>
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            ...
                            <input type="text" value={t.stock} onChange={(e) => actualizarStock(t.id, e.target.value)} className="bg-stone-950 p-1 rounded border border-stone-800 w-20 text-xs text-center outline-none" />
                        </div>
                      </div>
                    </div>
                  ))
        ...
        {esAdmin && vista === 'catalogo-avios' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {avios.length === 0 ? <p className="col-span-full text-stone-500 text-center py-10 italic">No existen avíos registrados.</p> : (
                  avios.map(a => (
                    <div key={a.id} className="bg-stone-900/40 backdrop-blur-md border border-stone-800 rounded-3xl overflow-hidden relative">
                      <button onClick={(e) => { e.stopPropagation(); setModalConfirm({ isOpen: true, text: "¿Estás segura de que quieres eliminar este avío?", action: () => borrarAvio(a.id) }); }} className="absolute top-2 right-2 text-white bg-black/50 p-2 rounded-full hover:bg-red-900 z-10 text-xs">✕</button>
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            <span className="text-xs text-stone-400">Stock:</span>
                            <input type="text" value={a.stock} onChange={(e) => actualizarStockAvio(a.id, e.target.value)} className="bg-stone-950 p-1 rounded border border-stone-800 w-20 text-xs text-center outline-none" />
                        </div>
                      </div>
                    </div>
                  ))
        ...
        {esAdmin && vista === 'clientes' && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {clientesFiltrados.length === 0 ? <p className="col-span-full text-stone-500 text-center py-10 italic">No existen clientes.</p> : (
                  clientesFiltrados.map(c => (
                    <div key={c.id} className="bg-stone-900/40 backdrop-blur-md border border-stone-800 p-6 rounded-3xl relative">
                      <button onClick={(e) => { e.stopPropagation(); setModalConfirm({ isOpen: true, text: "¿Estás segura de que quieres eliminar este cliente?", action: () => borrarCliente(c.id) }); }} className="absolute top-4 right-4 text-stone-600 hover:text-red-400">✕</button>
          ...
          <>
            <div className="bg-stone-900/40 backdrop-blur-md border border-stone-800 p-6 md:p-8 rounded-3xl max-w-2xl mx-auto relative">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                <button onClick={() => cambiarVista('editar-cliente')} className="bg-stone-800 px-4 py-2 rounded-xl text-xs border border-stone-700 hover:bg-stone-700 font-medium">Editar Datos y Medidas</button>
                <button onClick={() => window.print()} className="bg-stone-800 px-4 py-2 rounded-xl text-xs border border-stone-700 hover:bg-stone-700 font-medium">Imprimir Ficha</button>
                <button onClick={() => setModalConfirm({ isOpen: true, text: "¿Estás segura de que quieres eliminar este cliente?", action: () => borrarCliente(clienteSeleccionado.id) })} className="bg-red-950/40 text-red-400 px-4 py-2 rounded-xl text-xs border border-red-900/50 hover:bg-red-900/40 font-medium">Eliminar Cliente</button>
              </div>
              ...
              <div className="space-y-4">
                {pedidos.filter(p => p.cliente === clienteSeleccionado.nombre).length === 0 ? <p className="text-stone-500 text-xs italic">No hay pedidos registrados.</p> : (
                  pedidos.filter(p => p.cliente === clienteSeleccionado.nombre).map(p => {
                    ...
                    <div key={p.id} className={`bg-stone-950/40 border p-4 rounded-2xl flex flex-col gap-3 relative ${esRechazado ? 'border-red-900/50' : 'border-stone-800'}`}>
                      <button onClick={() => setModalConfirm({ isOpen: true, text: "¿Estás segura?", action: () => borrarPedidoDefinitivo(p.id) })} className="absolute top-4 right-4 text-stone-600 hover:text-red-400 text-xs">✕</button>
                      ...
                      {p.descripcionDetalle && <p className="text-xs text-stone-300 bg-stone-900/60 p-2.5 rounded-xl border border-stone-800"><strong>Detalles:</strong> {p.descripcionDetalle}</p>}
                      {esRechazado && p.motivoRechazo && <p className="text-xs text-red-300 bg-red-950/40 p-2 rounded-xl"><strong>Motivo:</strong> {p.motivoRechazo}</p>}
            ...
            <div className="print-ficha-exclusiva hidden">
              <div className="border-b-2 border-black pb-4 mb-6"><h1 className="text-3xl font-bold text-black">ATELIER - FICHA DE CLIENTE</h1></div>
              <div className="mb-6 space-y-1"><p className="text-xl font-bold text-black">Cliente: {clienteSeleccionado.nombre}</p><p className="text-sm text-gray-700">Teléfono: {clienteSeleccionado.telefono}</p></div>
              <h3 className="text-md font-bold uppercase border-b border-gray-400 pb-1 mb-4 text-black">Medidas Registradas</h3>
              <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
                {Object.entries(clienteSeleccionado.medidas || {}).map(([k, v]) => (
                  <div key={k} className="flex justify-between border-b border-gray-200 py-1.5"><span className="text-gray-800 font-medium">{k}:</span><span className="font-bold text-black">{v || 'N/A'}</span></div>
                ))}
              </div>
            </div>
          </>
        ...
        {esAdmin && vista === 'calculadora' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            <div className="bg-stone-900/40 backdrop-blur-md border border-stone-800 p-6 md:p-8 rounded-3xl md:col-span-2">
              ...
              <div className="mb-4">
                <label className="block text-xs text-stone-400 mb-1">Cargar precio desde Catálogo de Telas:</label>
                <select onChange={e => {
                    const telaEncontrada = telas.find(t => t.nombre === e.target.value);
                    if (telaEncontrada && telaEncontrada.precio) setCalc(prev => ({ ...prev, costoMetro: Number(telaEncontrada.precio) }));
            ...
            <div className="bg-stone-900/40 backdrop-blur-md border border-stone-800 p-6 rounded-3xl flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between text-stone-400"><span>Costo Materiales:</span><span className="text-white">${materiales.toLocaleString()}</span></div>
                  <div className="flex justify-between text-stone-400"><span>Mano de Obra:</span><span className="text-white">${manoObra.toLocaleString()}</span></div>
                  <div className="flex justify-between text-stone-400 border-t border-stone-800/50 pt-2"><span>Costo Total:</span><span className="text-white">${costoTotal.toLocaleString()}</span></div>
                  <div className="flex justify-between text-stone-400"><span>Precio Venta:</span><span className="text-white">${precioFinal.toLocaleString()}</span></div>
                </div>
              </div>
        ...
        {esAdmin && vista === 'ganancias' && (
          <div className="bg-stone-900/40 backdrop-blur-md border border-stone-800 p-6 md:p-8 rounded-3xl max-w-3xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={exportarReportePDF} className="bg-white text-stone-950 px-4 py-2.5 rounded-xl text-xs font-bold hover:bg-stone-200 transition-colors">
                🖨️ Exportar Reporte (PDF)
              </button>
            </div>

            {Object.keys(gananciasPorMes).length === 0 ? <p className="text-stone-500 text-center py-10 italic">No hay pedidos facturados.</p> : (
                  ...
                  <div key={mes} className="bg-stone-950/60 border border-stone-800 p-5 rounded-2xl">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-stone-800 pb-4 mb-4">
                      ...
                      <div className="flex gap-6 text-right">
                        <div><span className="block text-[10px] text-stone-500 uppercase">Ingresos</span><span className="text-base font-semibold">${datos.ingresos.toLocaleString()}</span></div>
                        <div><span className="block text-[10px] text-stone-500 uppercase">Ganancia Neta</span><span className="text-xl font-bold text-emerald-400">${datos.ganancia.toLocaleString()}</span></div>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-stone-900 border border-stone-800 p-6 md:p-8 rounded-3xl max-w-sm w-full shadow-2xl">
            ...
            <div className="space-y-3 mb-6">
              <div>
                ...
                <input type="number" min="1" value={montoPagoInput} onChange={(e) => setMontoPagoInput(e.target.value)} placeholder="Ej: 15000" className="w-full bg-stone-950 p-3 rounded-xl border border-stone-800 text-sm text-white outline-none" autoFocus />
              </div>
              <div>
                <label className="text-xs text-stone-500 pl-1">Método de pago</label>
                <select value={metodoPagoInput} onChange={(e) => setMetodoPagoInput(e.target.value)} className="w-full bg-stone-950 p-3 rounded-xl border border-stone-800 text-sm text-white outline-none">
            ...
            <div className="flex gap-3">
              <button onClick={() => setModalPago({ isOpen: false, pedidoId: null })} className="flex-1 bg-stone-800 text-white py-3 rounded-xl font-bold text-xs">Cancelar</button>
              <button onClick={registrarPagoParcial} className="flex-1 bg-white text-stone-950 py-3 rounded-xl font-bold text-xs">Aceptar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalRechazo.isOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/90 p-4">
          <div className="bg-stone-900 border border-stone-800 p-6 md:p-8 rounded-3xl max-w-sm w-full text-center shadow-2xl">
            ...
            <textarea rows="3" value={modalRechazo.motivo} onChange={(e) => setModalRechazo(prev => ({ ...prev, motivo: e.target.value }))} placeholder="Ej: Taller saturado..." className="w-full bg-stone-950 p-3 rounded-xl border border-stone-800 text-sm text-white outline-none mb-6 resize-none" required />
            <div className="flex gap-3">
              <button onClick={() => setModalRechazo({ isOpen: false, pedidoId: null, motivo: '' })} className="flex-1 bg-stone-800 text-white py-3 rounded-xl font-bold text-xs">Cancelar</button>
              <button onClick={confirmarRechazoAdmin} className="flex-1 bg-red-950 text-red-300 border border-red-900/50 py-3 rounded-xl font-bold text-xs">Confirmar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {fotoAmpliada && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4" onClick={() => setFotoAmpliada(null)}>
          <button className="absolute top-6 right-6 text-white bg-stone-900/50 p-2 rounded-full z-[101]" onClick={() => setFotoAmpliada(null)}>✕</button>
          <img src={fotoAmpliada} alt="Ampliada" className="max-w-full max-h-[90vh] object-contain rounded-xl shadow-2xl" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
            ...
            {modalConfirm.buttons ? (
              <div className="flex flex-col gap-3">
                {modalConfirm.buttons.map((btn, idx) => (
                  <button key={idx} onClick={() => { btn.action(); setModalConfirm({ isOpen: false, text: '', action: null, buttons: null }); }} className={`w-full py-3 rounded-xl font-bold ${btn.style}`}>{btn.text}</button>
                ))}
                <button onClick={() => setModalConfirm({ isOpen: false, text: '', action: null, buttons: null })} className="w-full mt-2 text-stone-500 hover:text-white text-sm py-2">Cancelar</button>
              </div>
            ) : (
              <div className="flex gap-4">
                <button onClick={() => setModalConfirm({ isOpen: false, text: '', action: null, buttons: null })} className="flex-1 bg-stone-800 text-white py-3 rounded-xl font-bold">Cancelar</button>
                <button onClick={() => { if (modalConfirm.action) modalConfirm.action(); setModalConfirm({ isOpen: false, text: '', action: null, buttons: null }); }} className="flex-1 bg-red-950/40 text-red-400 py-3 rounded-xl font-bold border border-red-900/50">Eliminar</button>
              </div>
            )}
          </div>
        </div>
